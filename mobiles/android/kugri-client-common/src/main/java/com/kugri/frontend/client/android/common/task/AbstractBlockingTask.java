 ...
 */
public abstract class AbstractBlockingTask extends AsyncTask<AbstractRequest, Void, AbstractResponse>{
	 ...
	 */
	public AbstractBlockingTask( int timeout) {
	...
	protected AbstractResponse doInBackground(AbstractRequest... params) {
		...
		this.request.getHandler().sendEmptyMessage(RequestCode.PROGRESS_STATIC_INITIATE);
		...
		try{
			...
			Context.LOGGER.pushErrors(e, "AbstractBlockingTask" + Log.getStackTraceString(e));
			this.failed = true;
		}
	...
	protected void onPostExecute(AbstractResponse result) {
		...
		this.request.getHandler().sendEmptyMessage(RequestCode.PROGRESS_STATIC_END);
		if(this.request != null && this.response != null){
			Context.LOGGER.pushDebugs("AbstractBlockingTask Request: "+this.request.toJson());
			Context.LOGGER.pushDebugs("AbstractBlockingTask Response: "+this.response.toJson());
			...
			Context.LOGGER.pushErrors(new Exception(), "AbstractBlockingTask" + (this.request == null?"Request is null and ":"")+ (this.response == null?"Response is null":""));
			this.failed = true;
		}
	}
